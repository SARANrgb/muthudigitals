...
export default function ContactView({ useTamil }: ContactViewProps) {
  ...
  const [serviceType, setServiceType] = useState('Package 1 - Essential (₹50,000)');
  const [customMsg, setCustomMsg] = useState('');
  const [showStatusSuccess, setShowStatusSuccess] = useState(false);

  // Handle Form Submission - Opens WhatsApp chat box with pre-formatted message
    ...
    <div id="contact-view" className="space-y-16 sm:space-y-24">
      ...
      <section className="text-center max-w-2xl mx-auto space-y-4">
        ...
        <h2 className="font-serif text-4xl sm:text-5xl font-black text-white gold-gradient-text leading-tight">
          {useTamil ? 'தொடர்பு கொள்ள' : 'Contact Us'}
        </h2>
        <p className="font-sans text-sm sm:text-base text-gray-400 leading-relaxed">
          {useTamil 
            ? 'கீழே உங்கள் விவரங்களைத் தேர்ந்தெடுக்கவும். இது உங்களது விசாரணையை வாட்ஸ்அப்பில் அனுப்ப உதவும்.'
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Booking Request Form (WhatsApp Message Builder) */}
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h4 className="font-sans text-xs font-bold text-gray-400 uppercase">
                    {useTamil ? 'முதன்மை ஸ்டு튜디오 முகவரி' : 'Main Studio Address'}
                  </h4>
                  <a
